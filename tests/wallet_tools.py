...
class WalletTool:
    ...
    def generate_signed_transaction(
        ...
        if condition_dic is None:
            condition_dic = {}
        transaction = self.generate_unsigned_transaction(amount, new_puzzle_hash, [coin], condition_dic, fee, additional_outputs=additional_outputs)
    ...
    def generate_signed_transaction_multiple_coins(
        ...
        if condition_dic is None:
            condition_dic = {}
        transaction = self.generate_unsigned_transaction(amount, new_puzzle_hash, coins, condition_dic, fee, additional_outputs=additional_outputs)
        assert transaction is not None
        return self.sign_transaction(transaction)
